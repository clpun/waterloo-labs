...
OS_RESULT rt_tsk_get (OS_TID task_id, RL_TASK_INFO *p_task_info) {
  ...
  p_task = os_active_TCB[task_id-1];
  task_info.state = p_task->state;
  task_info.prio = p_task->prio;
  task_info.task_id = p_task->task_id;
  task_info.stack_usage = ((BLK_SIZE-(U32)(p_task->stack - p_task->tsk_stack))/8)*100;
  task_info.ptask = p_task->ptask;

  p_task_info = &task_info;
  return (OS_R_OK);
}
