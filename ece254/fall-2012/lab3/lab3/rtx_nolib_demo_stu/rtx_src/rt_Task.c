...
#include "rt_List.h"
#include "rt_MemBox.h"
#include "rt_Mem.h"                                  /* added in ECE254 lab3 */
#include "rt_Robin.h"
#include "rt_HAL_CM.h"
...
void rt_sys_init (FUNCP first_task, U32 prio_stksz, void *stk) {
  ...
  /* Added in ECE254 lab3 
    Initialize dynamic memory pool outside the image 
  */
  if (rt_mem_init(NUM_BLKS, BLK_SIZE) == OS_R_NOK) {
    os_error(OS_ERR_MEM_INI);
  }
...
OS_RESULT rt_tsk_get (OS_TID task_id, RL_TASK_INFO *p_task_info) {
	...
	if (task_id == os_tsk.run->task_id) {
		U32 current_pointer;
		current_pointer = rt_get_PSP();
		p_task_info->state = os_tsk.run->state;
		p_task_info->prio = os_tsk.run->prio;
		p_task_info->task_id = os_tsk.run->task_id;
		p_task_info->stack_usage = 100-(((current_pointer - (U32)os_tsk.run->stack)*100 )/(size*4));
		p_task_info->ptask = os_tsk.run->ptask;
			
		return (OS_R_OK);
	}
	
	if (task_id == 255) {
		p_task_info->state = os_idle_TCB.state;
		p_task_info->prio = os_idle_TCB.prio;
		p_task_info->task_id = os_idle_TCB.task_id;
		p_task_info->stack_usage = 100-((((U32)os_idle_TCB.tsk_stack - (U32)os_idle_TCB.stack)*100 )/(size*4));
		p_task_info->ptask = os_idle_TCB.ptask;
	...
	p_task_info->stack_usage = 100-((((U32)p_task->tsk_stack - (U32)p_task->stack)*100 )/(size*4));
