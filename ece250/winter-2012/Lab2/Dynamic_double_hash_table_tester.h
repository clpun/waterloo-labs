/*************************************************
 * Dynamic_double_hash_table_tester
 * A class for testing double hash tables.
 *
 * Author:  Douglas Wilhelm Harder
 * Copyright (c) 2007-9 by Douglas Wilhelm Harder.  All rights reserved.
 *
 * DO NOT EDIT THIS FILE
 *************************************************/

#ifndef DYNAMIC_DOUBLE_HASH_TABLE_TESTER_H
#define DYNAMIC_DOUBLE_HASH_TABLE_TESTER_H
...
class Dynamic_double_hash_table_tester:public Tester< Dynamic_double_hash_table<Type> > {
	using Tester< Dynamic_double_hash_table<Type> >::object;
	using Tester< Dynamic_double_hash_table<Type> >::command;

	public:
		Dynamic_double_hash_table_tester( Dynamic_double_hash_table<Type> *obj = 0 ):Tester< Dynamic_double_hash_table<Type> >( obj ) {
			// empty
		}
 ...
 *   deleted_factor d  deleted_factor  the deleted factor equals d
...
template <typename Type>
void Dynamic_double_hash_table_tester<Type>::process() {
	if ( command == "size" ) {
	...
	} else if ( command == "deleted_factor" ) {
		// check if the deleted factor equals the next double read

		double expected_deleted_factor;

		std::cin >> expected_deleted_factor;

		double actual_deleted_factor = object->deleted_factor();

		if ( actual_deleted_factor == expected_deleted_factor ) {
			std::cout << "Okay" << std::endl;
		} else {
			std::cout << "Failure in deleted_factor(): expecting the value '" << expected_deleted_factor << "' but got '" << actual_deleted_factor << "'" << std::endl;
		}
